...
public class DeleteOpJdbcTest {
    ...
    public void prepare() {
        ...
        insert.intoTable(person).columnValues(asList(set(fname,"Tina"), set(lname,"Turner"), set(email,"tina@rediffmail.com"), set(adult, true), set(age, 32)));
        try (Connection connection = util.getConnection();
             PreparedStatement ps = connection.prepareStatement(insert.getSQL())) {
            insert.fill(ps).addBatch();
            insert = InsertOp.create().intoTable(person).columnValues(asList(set(fname,"Angelina"), set(lname,"Jolie"), set(email,"angel@rediffmail.com"), set(adult, true), set(age,27)));
            insert.fill(ps).addBatch();
            ps.executeBatch();
            //connection.close();
    ...
    public void deleteWhereOrConditon(){
        ...
        operation.deleteFrom(person).where().anyColumnValues(set(fname,"Tina"), set(fname,"Angelina"));
        try(Connection connection = util.getConnection();
